...
class conexionAdo
{
	...
	function conectar($driver,$server,$user,$pws,$port,$sid)
	{
		if($driver=='mysql'){ 
		    ...
		    $this->dsn = $driver.'://'.$user.':'.$pws.'@'.$server.':'.$port.'/'.$sid; //Conexion con PostgreSQL			
			...
			$this->conn->Connect($server, $user, $pws, $sid);	
                        ...
                        $this->dsn2=$driver.'://'.$user.':'.$pws.'@'.$server.'/'.$sid.'?persist&dialect=3';
                       // print $this->dsn2;exit;
                       // $this->pantera = $driver.'://'.$user.':'.$pws.'@'.$server.':'.$port.'/'.$sid.'?persist';
 			$this->conn = ADONewConnection($this->dsn2); //Conexion con InterBase
