...
@$aliados = intval($_GET["al"]);

	$result = array();
	$data = Mactivo::find_by_sql("select id,cid,mmarcas_id,modelo,serial,activo,maliados_id,status,comodato from mactivos WHERE status=1 AND maliados_id=".$aliados." order by id desc;");
	$items = 0;
