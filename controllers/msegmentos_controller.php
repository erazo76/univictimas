  ...
  case 'add':
      ...
      if($nombre ==""){
        ...
        $respuesta = array('resultado'=>'error','mensaje'=>'<div class="alert alert-warning alert-dismissable">
            ...
            Ingrese el nombre del semento.
            </div>');

      }else{  //(A) si se escribe un nombre se consulta si ya existe...

        $consulta = Msegmento::find('all',array('conditions' => array('nombre=?',$nombre)));
          ...
          if($consulta==null){  //(B) si no existe lo guarda...
            ...
            if($estado!=1){ //se activa mensaje modal para llevar el status a "1" ... (o no).
              ...
              $respuesta = array('resultado'=>'alert','mensaje'=>'<input type="hidden" id="id" value="'.$ide.'">
                  ...
                  Registro inactivo . ¿Desea incluir ['.$nombre.'] en el maestro de segmentos?
              ...
              $respuesta = array('resultado'=>'alert','mensaje'=>'<div class="alert alert-danger alert-dismissable">
                  ...
                  El segmento ['.$nombre.'] ya se encuentra registrado.
                  </div>');
      ...
      if($consulta==null){
            ...
            if($estado!=1){ //se activa mensaje modal para llevar el status a "1" ... (o no).
              ...
              $respuesta = array('resultado'=>'alert','mensaje'=>'<input type="hidden" id="id" value="'.$ide.'">
                  ...
                  Registro inactivo . ¿Desea incluir ['.$nombre.'] en maestro de segmentos?
...
#*******************************************************************************
  case 'search':
    ...
    if($record !=null){
      ...
      @$data = Msegmento::find('all',array('conditions' => array('id=?',$record)));
...
#******************************************************************************
    ...
    case 'get_segmentos':
        ...
        @$data = Msegmento::find('all',array('conditions' => array('status=?',1),'order' => 'id desc'));

      if($data !=null){
               $resp = '<option value=""selected disabled>Indique el segmento</option>';
            foreach($data as $rs){
