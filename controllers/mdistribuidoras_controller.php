...
switch ($action){
  ...
  case 'add':
      ...
      if($nombre ==""){
        ...
        $consulta = Mdistribuidora::find('all',array('conditions' => array('nombre=?',$nombre)));
          ...
          if($consulta==null){  //(B) si no existe lo guarda...
            ...
            $distri = new Mdistribuidora();
            $distri->nombre = $nombre;
            $distri->mregiones_id = $region;
            $distri->user_create = $usuario_id;
            $distri->created = $hoy;


            if($distri->save()){ // da el mensaje de guardado...
  ...
  case 'edit':
    ...
    $consulta = Mdistribuidora::find('all',array('conditions' => array('nombre=?',$nombre)));
      ...
      if($consulta==null){
            ...
            $distri= Mdistribuidora::find($id);
            $distri->nombre = $nombre;
            $distri->mregiones_id = $region;
            $distri->user_modify = $usuario_id;
            $distri->updated = $hoy;
  ...
  case 'delete':
      ...
      if($record !=null){
        ...
        if($usuario_id !="" && ($rol ==2 || $rol==4)){
                  ...
                  $distri= Mdistribuidora::find($record);
                  $distri->user_modify = $usuario_id;
                  $distri->updated = $hoy;
                  $distri->status = 0;
...
#*******************************************************************************
  case 'search':
    ...
    if($record !=null){
      ...
      @$data = Mdistribuidora::find('all',array('conditions' => array('id=?',$record)));
...
#******************************************************************************
    ...
    if($rol ==3 ){
      ...
      @$data = Musuario::find('all',array('conditions' => array('id=?',$usuario_id)));

      if($data !=null){

        foreach($data as $rs){

          $resp = $rs->mdistribuidoras_id;
