...
switch ($action){
  ...
  case 'contar_id':
    //session_start();
    //$usuario_id = $_SESSION['idusuariox'];
    $data = Madjuntaexcel::find('all',array('conditions' => array('no_soli=?',$idea3)));
...
/*############################################################################################################-*/
  ...
  case 'contar_id2':
    ...
    $data = Madjuntaexcel::find('all',array('conditions' => array('no_soli=?',$guarda)));
...
#*******************************************************************************
  ...
  if($record !=null){
        ...
        if($usuario_id !="" /*&& ($rol ==1 || $rol==4)*/){
                  ...
                  $acti= Madjuntaexcel::find($record);
                  unlink('../dist/adjunto/adjuntos/'.$acti->adjunto); 
                  if($acti->delete()){
...
#*******************************************************************************
  case 'search':
    if($record !=null){
      ...
      @$data = Madjuntaexcel::find('all',array('conditions' => array('no_soli=?',$record)));
...
#******************************************************************************
 ...
 #******************************************************************************
    ...
    case 'temporal':
      ...
      @$consulta1 = Madjunto::find('all',array('conditions' => array('adjunto=?',$comodin)));
